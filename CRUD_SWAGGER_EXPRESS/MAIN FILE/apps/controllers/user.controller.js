...
exports.update = async (req, res) => {
    ...
    await User.update(req.body, {where: {id:id}})
              .then(num => {
                if (num == 1) res.status(200).send({message: `ID - ${id} is updated`});
                else res.status(404)
...
exports.deleteAll = async (req, res) => {
    ...
    await User.destroy({
        ...
        .then(() => {res.send({message: `All users are deleted`});})
        .catch(error => {
            res.status(500)
...
exports.deleteUserRequest = async (req, res) => {
    ...
    .then(() => res.status(200).send(`Deleted all request for ${id}`))
    .catch(error => {
        res.status(500)
        .send(`Error occured while deleting the user requests (${error})`);
        });
};
